...
this.clickListener();this.closeListener();this.inputListener();this.holdonFocus();this.checkboxControl()},detailshow:function(id,data_arr,multiple,width,top,left){let type='checkbox';if(multiple){type='checkbox'}else{type='radio'}
...
canScrollAjax=!0;searchtext=null;$(this).blur()}})},holdonFocus:function(){$('html').on('blur.cocoAjaxMultiSelect',"#"+this.$element.attr('id')+"[type='cocoAjaxMultiSelect']",function(){if($(this).attr('focus')=='on'){$(this).focus()}})},checkboxControl:function(){var _this=this;let multiple=this.$element.attr('multiple');$('html').on('click.cocoAjaxMultiSelect',".ajaxselect_detail[for='"+this.$element.attr('id')+"'] input",function(){let is_checked=$(this).attr('checked');let value=$(this).next('label').text();if(is_checked=='checked'){if(multiple){for(let i=0;i<selectedval.length;i++){if(selectedval[i]==value){selectedval.splice(i,1);i--}}}}else{if(multiple){selectedval.push(value)}else{selectedval=new Array(value)}}
_this.settings.checkedCode(selectedval)})}});$.fn[pluginName]=function(options){return this.each(function(){if(!$.data(this,"plugin_"+pluginName)){$.data(this,"plugin_"+pluginName,new cocoAjaxMultiSelect(this,options))}})}})(jQuery)